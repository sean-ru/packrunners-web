        ...
        init : function(args) {
            angular
                ...
                .config(function($routeProvider) {
                    $routeProvider.when("/", {
                        templateUrl: args.contextPath + '/.resources/studyguides/webresources/views/find-study-guide-results.html',
                ...
                .controller('MainController', ['$scope', '$routeParams', '$http', '$location', function($scope, $routeParams, $http, $location) {
                    ...
                    $http.get(args.restBase + '/studyGuides/v1/?lang=' + args.language).then(function(response) {
                        $scope.courses = response.data.results;
                    }, function(response) {
                        console.error("Couldn't reach endpoint [/studyGuides/v1/].");
                    });
                    ...
                    $scope.$watch(function() {
                        ...
                        if (newValues !== oldValues && newValues.courseTypes) {
                            ...
                            qs = encodeURI(args.restBase + '/studyGuides/v1/' + qs);
                            $http.get(qs).then(function(response) {
                                $scope.filteredStudyGuides = response.data.results;
                            });
                        }
                    }, true);
                }]);
