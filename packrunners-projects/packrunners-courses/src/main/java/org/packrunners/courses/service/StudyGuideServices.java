...
public class StudyGuideServices {
  ...
  @Inject
  public StudyGuideServices(CoursesModule coursesModule, TemplateTypeHelper templateTypeHelper,
  ...
  public StudyGuide marshallStudyGuideNode(Node studyGuideNodeRaw) {
    ...
    if (studyGuideNodeRaw != null) {
      ...
      try {
        ...
        if (studyGuideNode.hasProperty(StudyGuide.PROPERTY_NAME_COURSE_NAMES)) {
          final List<Category> courseNames = getCategories(studyGuideNode,
              StudyGuide.PROPERTY_NAME_COURSE_NAMES);
  ...
  public List<StudyGuide> getStudyGuidesByCategory(String categoryPropertyName, String identifier,
    ...
    try {
      final Session session = MgnlContext.getJCRSession(CoursesModule.STUDY_GUIDES_REPOSITORY_NAME);
